#!/usr/bin/env python3
"""Render Reel 0005 with documented existing visual assets after fresh-media quota exhaustion."""
...
REEL = PROJECT / "reels-production/batches/Batch_001/Reel_0005"
SOURCES = [
...
ASSETS = REEL / "assets"
TMP = REEL / "video" / "render_tmp"
AUDIO = REEL / "voice" / "Reel_0005_hi_final_60s.wav"
SRT = REEL / "captions" / "voiceover_hi_final.srt"
OUT = REEL / "video" / "Reel_0005_final_1080x1920.mp4"
...
def main() -> int:
    ...
    if any(not item.exists() or item.stat().st_size == 0 for item in required):
        print("Required documented fallback asset unavailable", file=sys.stderr)
        return 2
    ...
    for number, source in enumerate(SOURCES, 1):
        ...
        zoom = "min(zoom+0.000060,1.080)" if number % 2 else "min(zoom+0.000038,1.055)"
        vf = (
            ...
            "eq=contrast=1.06:saturation=0.78:brightness=-0.02,format=yuv420p"
        )
        ...
        clips.append(clip)
    sequence = [clips[0], clips[1], clips[2], clips[3], clips[4], clips[0]]
